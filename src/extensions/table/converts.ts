...
const parseColgoupData = (colgroupNode) => {
  ...
  return Array.prototype.map.call(
    colgroupNode.querySelectorAll('col'),
    (colNode) => {
      return {
...
const buildColgroup = (blockData) => {
  if (blockData?.colgroupData?.length) {
    return `<colgroup>${blockData.colgroupData
      .map((col) => `<col width="${col.width}"></col>`)
      .join('')}</colgroup>`
  }
...
export const tableImportFn = (nodeName, node) => {
  if (
    ...
    node &&
    node.querySelector &&
    node.querySelector(':scope > table')
  ) {
