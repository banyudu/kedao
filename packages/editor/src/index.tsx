...
(EditorState.prototype as any).setConvertOptions = function setConvertOptions (
  options = {}
) {
  this.convertOptions = options
};

(EditorState.prototype as any).toHTML = function toHTML (options = {}) {
  const convertOptions = this.convertOptions || {}
  return convertEditorStateToHTML(this, { ...convertOptions, ...options })
};

(EditorState.prototype as any).toRAW = function toRAW (noStringify) {
  return noStringify
    ? convertEditorStateToRaw(this)
    : JSON.stringify(convertEditorStateToRaw(this))
};

(EditorState.prototype as any).toText = function toText () {
  return this.getCurrentContent().getPlainText()
};

(EditorState.prototype as any).isEmpty = function isEmpty () {
  return !this.getCurrentContent().hasText()
};

(EditorState as any).createFrom = (content, options = {}) => {
  const customOptions: any = { ...options }
  customOptions.unitExportFn =
    customOptions.unitExportFn ||
    (KedaoEditor as any).defaultProps.converts.unitExportFn
  ...
  if (typeof content === 'string') {
    try {
      if (/^(-)?\d+$/.test(content)) {
        ...
        editorState = (EditorState as any).createFrom(
          JSON.parse(content),
          customOptions
        )
      }
...
(KedaoEditor as any).createEditorState = (EditorState as any).createFrom
