...
const createCellBlock = (cell) => {
  ...
  const { tableKey, key, colIndex, rowIndex, colSpan, rowSpan, text, isHead } =
    cell
  ...
  return new ContentBlock({
    ...
    characterList: Immutable.List(
      Immutable.Repeat(CharacterMetadata.create(), text.length)
    )
  })
}
...
const createRowBlocks = (tableKey, rowIndex, rowLength, firstCellText = '') => {
  const cells = Immutable.Range(0, rowLength)
    .map((index) => {
      const cellBlock = createCellBlock({
        tableKey: tableKey,
        colIndex: index,
        rowIndex: rowIndex,
...
const updateTableBlocks = (
  ...
  const blocksAfter = contentBlocks
    .skipUntil((block, key) => {
      const nextBlockKey = contentState.getKeyAfter(key)
      return (
        block.getData().get('tableKey') === tableKey &&
        nextBlockKey &&
        contentState.getBlockForKey(nextBlockKey).getData().get('tableKey') !==
          tableKey
      )
    })
    .rest()
...
export const rebuildTableBlocks = (tableBlocks, addonBlockData = {}) => {
  ...
  return tableBlocks.map((block) => {
    ...
    const rowIndex: number = blockData.get('rowIndex')
    const colSpan: number = blockData.get('colSpan') || 1
    const rowSpan: number = blockData.get('rowSpan') || 1
    ...
    for (
      ;
      skipedCells[rowIndex]?.[colIndex];
      colIndex++, cellCountOfRow[rowIndex]++
    );
...
export const rebuildTableNode = (tableNode) => {
  ...
  [].forEach.call(tableNode.rows, (row, rowIndex) => {
    [].forEach.call(row.cells, (cell, cellIndex) => {
      ...
      for (; skipedCells[rowIndex]?.[colIndex]; colIndex++) {
        /* _ */
      }
...
export const updateAllTableBlocks = (editorState: EditorState, tableKey, blockData) => {
  ...
  const nextContentState = updateTableBlocks(
    contentState,
...
export const getCellCountForInsert = (tableBlocks, rowIndex) => {
  return findBlocks(tableBlocks, 'rowIndex', rowIndex).reduce(
    (count: number, block) => {
      return count + ((block.getData().get('colSpan') as number) || 1) * 1
    },
    0
  )
}
...
export const getCellsInsideRect = (
  ...
  tableBlocks.forEach((block) => {
    ...
    const colIndex: number = blockData.get('colIndex')
    const rowIndex: number = blockData.get('rowIndex')
    const colSpan: number = blockData.get('colSpan')
    const rowSpan: number = blockData.get('rowSpan')

    matchedCellLocations.forEach(([x, y]) => {
      if (colIndex === x && rowIndex === y) {
        !matchedCellBlockKeys.includes(blockKey) &&
          (matchedCellBlocks = matchedCellBlocks.push(block)) &&
          matchedCellBlockKeys.push(blockKey);
        (colSpan > 1 || rowSpan > 1) &&
          !spannedCellBlockKeys.includes(blockKey) &&
          (spannedCellBlocks = spannedCellBlocks.push(block)) &&
          spannedCellBlockKeys.push(blockKey)
...
export const insertCell = (tableBlocks, cell) => {
  ...
  const blocksBefore = tableBlocks.takeUntil((block) => {
    ...
    return (
      (blockColIndex >= colIndex && blockRowIndex === rowIndex) ||
      blockRowIndex > rowIndex
    )
  })
  ...
  const nextTableBlocks = blocksBefore.concat(
    Immutable.OrderedMap([[cellBlock.getKey(), cellBlock]]).toSeq(),
    blocksAfter
  )
...
export const insertTable = (editorState: EditorState, columns = 3, rows = 3) => {
  ...
  const nextContentBlocks = blocksBefore
    .concat(
...
// 删除整个表格
export const removeTable = (editorState: EditorState, tableKey) => {
  if (!tableKey) {
    return editorState
  }
  ...
  const focusCellKey = (
    tableBlocks.first()
      ? contentState.getBlockBefore(tableBlocks.first().getKey()) ||
        nextContentBlocks.first()
      : nextContentBlocks.first()
  ).getKey()
  ...
  return EditorState.push(editorState, nextContentState, 'remove-table' as any)
}
...
export const insertColumn = (editorState: EditorState, tableKey, cellCounts, colIndex) => {
  ...
  const nextTableBlocks = tableBlocks.map((block) => {
    ...
    const nextColIndex =
      blockColIndex < colIndex ? blockColIndex : blockColIndex + 1
    const nextColSpan =
      blockColIndex < colIndex &&
      blockColSpan > 1 &&
      blockColIndex + blockColSpan > colIndex
        ? blockColSpan + 1
        : blockColSpan

    const needUpdate =
      nextColIndex !== blockColIndex || nextColSpan !== blockColSpan

    if (
      (blockColSpan === 1 && blockColIndex === colIndex - 1) ||
      (blockColSpan > 1 && blockColIndex + blockColSpan === colIndex)
    ) {
  ...
  const nextContentState = updateTableBlocks(
    ...
    editorState.getSelection(),
    focusCellKey,
    insertCells(nextTableBlocks, cellsToBeAdded),
    tableKey
  )
...
// 从表格中移除指定的某一列单元格
export const removeColumn = (editorState: EditorState, tableKey, colIndex) => {
  const contentState = editorState.getCurrentContent()
  const contentBlocks = contentState.getBlockMap().toSeq()
  const tableBlocks = findBlocks(contentBlocks, 'tableKey', tableKey)

  const cellsToBeAdded = findBlocks(tableBlocks, 'colIndex', colIndex).reduce(
    (cellsToBeAdded, block) => {
  ...
  const nextTableBlocks = tableBlocks
    .filter((block) => {
    ...
    .map((block) => {
      ...
      const newColIndex =
        blockColIndex > colIndex ? blockColIndex - 1 : blockColIndex
      const newColSpan =
        blockColIndex < colIndex && blockColIndex + blockColSpan > colIndex
          ? blockColSpan - 1
          : blockColSpan
      const needUpdate =
        newColIndex !== blockColIndex || newColSpan !== blockColSpan
  ...
  const focusCellKey = (
    nextTableBlocks.first() ||
    contentState.getBlockBefore(tableBlocks.first().getKey()) ||
    contentState.getBlockAfter(tableBlocks.first().getKey())
  ).getKey()
  const nextContentState = updateTableBlocks(
    contentState,
    editorState.getSelection(),
...
export const insertRow = (
  ...
  const blocksBefore = findBlocks(tableBlocks, 'rowIndex', rowIndex, '<').map(
    (block) => {
      const blockData = block.getData().toJS()
      ...
      if (blockRowIndex > rowIndex) {
        ...
        const needUpdate =
          blockRowSpan && blockRowIndex + blockRowSpan > rowIndex
  ...
  const blocksAfter = findBlocks(tableBlocks, 'rowIndex', rowIndex, '>=').map(
    (block) => {
      const blockData = block.getData().toJS()
  ...
  const rowBlocks = createRowBlocks(
    tableKey,
    rowIndex,
    colCellLength || cellCounts
  ) as any
  const focusCellKey = rowBlocks.first().getKey()

  const nextTableBlocks = rebuildTableBlocks(
    blocksBefore.concat(rowBlocks, blocksAfter),
    addonBlockData
  )
  const nextContentState = updateTableBlocks(
    contentState,
...
export const removeRow = (editorState: EditorState, tableKey, rowIndex, addonBlockData = {}) => {
  ...
  const cellsToBeAdded = findBlocks(tableBlocks, 'rowIndex', rowIndex).reduce(
    (cellsToBeAdded, block) => {
      const { colIndex, rowIndex, colSpan, rowSpan } = block.getData().toJS()
  ...
  const nextTableBlocks = rebuildTableBlocks(
    insertCells(blocksBefore.concat(blocksAfter), cellsToBeAdded),
    addonBlockData
  )
  const nextContentState = updateTableBlocks(
    contentState,
...
// 合并单元格
export const mergeCells = (editorState: EditorState, tableKey, cellKeys) => {
  const contentState = editorState.getCurrentContent()
  const contentBlocks = contentState.getBlockMap()
  ...
  const tableBlocks = findBlocks(contentBlocks, 'tableKey', tableKey).filter(
    (block) => {
  ...
  const sortedCellBlocksData = cellBlocksData.sort(
    (prev, next) =>
      next.colIndex + next.rowIndex - (prev.colIndex + prev.rowIndex)
  )
  ...
  const mergedCell = contentState.getBlockForKey(firstCellData.key).merge({
    ...
    characterList: Immutable.List(
      Immutable.Repeat(CharacterMetadata.create(), mergedText.length)
    )
  })
  ...
  const nextContentState = updateTableBlocks(
    ...
    editorState.getSelection(),
    firstCellData.key,
    insertCell(tableBlocks, mergedCell),
    tableKey
  )
...
// 拆分单元格
export const splitCell = (editorState: EditorState, tableKey, cellKey) => {
  const contentState = editorState.getCurrentContent()
  const contentBlocks = contentState.getBlockMap()

  const cellsToBeAdded = []

  const tableBlocks = findBlocks(contentBlocks, 'tableKey', tableKey).map(
    (block) => {
      if (block.getKey() === cellKey) {
        ...
        for (let ii = colIndex; ii < colIndex + colSpan; ii++) {
  ...
  const nextContentState = updateTableBlocks(
    ...
    editorState.getSelection(),
    cellKey,
    insertCells(tableBlocks, cellsToBeAdded),
    tableKey
  )
